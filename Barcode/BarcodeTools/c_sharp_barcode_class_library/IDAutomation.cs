...
namespace CSharpExample
{
	...
	public class clsBarCode
	{
		...
		/// <summary>
		/// Method returns a Mod10 check digit given the input string.  
		/// </summary>
		/// <param name="input">string (must be numeric characters)</param>
		/// <returns>string checkdigit</returns>
		public string FindMod10Digit(string input)
		{
			//Make sure we only have digits first
			System.Text.StringBuilder GoodData = new System.Text.StringBuilder();
			for(int q = 0;q <= input.Length - 1;q++)
			{
				if(Char.IsDigit(input, q) == true)
					GoodData.Append(input, q, 1);
			}

			//If no digits, no calculation
			if(GoodData.Length > 0)
			{
				Char[] iArray = GoodData.ToString().ToCharArray();
				int evenSum = 0;
				int oddSum = 0;
				for (int i = 0; i < iArray.Length; i ++)
				{
					// Since C# indexes on zero, the first digit is actually considered "odd"
					if (i%2==0) 
					{
						// Odd
						oddSum += int.Parse(iArray[i].ToString());
					}
					else
					{
						// Even
						evenSum  += int.Parse(iArray[i].ToString());
					}
				}
				// Generate weighted total and find remainder of 10
				...
				{
					string rtrnValue = Convert.ToString(10 - remainder);
					return rtrnValue;
				}
			}
			else
		...
		//completed string 
		public string Code128(string InputValue)
		{
			...
			ChrArray = InputValue.ToCharArray(0,1);
			...
			if(AscCharacter > 31 && AscCharacter < 127)   
			{
			...
			if(LenDataToEncode > 4 && IsNumber(InputValue.Substring(1,4))==true)
			{	
				OutStartCharacter = (char) ASCII_START128_C;
				EncodingType = enuEncodingType.EncodingC; 
			}

			if(AscCharacter == ASCII_FNC1_128) //202 is the FNC1, with this Start C is mandatory
			{
				OutStartCharacter = (char) ASCII_START128_C;
				EncodingType = enuEncodingType.EncodingC;
			}
			...
			for(idx = 0;idx < LenDataToEncode;idx++)
			{
				...
				ChrArray = InputValue.ToCharArray(idx, 1);
				AscCharacter = (int) ChrArray[0];
				if(AscCharacter == ASCII_FNC1_128)
				{
				...
				//AND
				//AsciiFnc1Found = false  -- 
				if( ((idx < (LenDataToEncode - 4)) && (IsNumber(InputValue.Substring(idx,1)) == true) && 
					(IsNumber(InputValue.Substring(idx + 1,1)) == true) && 
					(IsNumber(InputValue.Substring(idx + 1,4)) == true )) || 
					( (idx < LenDataToEncode) && (IsNumber(InputValue.Substring(idx,1)) == true) && 
					(IsNumber(InputValue.Substring(idx + 1, 1)) == true) && (EncodingType == enuEncodingType.EncodingC) ) 
					&& (AsciiFnc1Found == false) )
				{	
					...
					CurrentChars = InputValue.Substring(idx, 2);
		...
		//of the code 
		public string Code128HR(string InputValue)
		{
			string HRData = "";				//This is the human readable text than will be returned
			int idx = 0;					//for loop coutner
			int LenDataToEncode = 0;		//Len of string so that we can do our loop
			char[] chrInputValue;			//character array of the inputvalue
			int CharAsciiValue = 0;			//Ascii value of a character
			string Current2Chars = "";		//This is the 2 characters after the FNC1 character
			int iCurrent2Chars = 0;			//Value of Current2Chars
			//Calculate the length of the string for our loop
			LenDataToEncode = InputValue.Length;
			chrInputValue = InputValue.ToCharArray(); 

			for(idx=0;idx<LenDataToEncode;idx++)
			{
				//Get the Ascii value of the character
				CharAsciiValue = (int) chrInputValue[idx];

				//Check for FNC1
				if( (idx < LenDataToEncode-2) && (CharAsciiValue == ASCII_FNC1_128) )
				{
					//It appears that there is an AI 
					//Get the value of each number pair (ex: 5 and 6 = 5*10+6 =56)
					//This is the next 2 characters after the fnc1 character 
					Current2Chars = InputValue.Substring(idx + 1, 2);
					iCurrent2Chars = Convert.ToInt32(Current2Chars);

					//Is this a 4 digit AI?.  First check if we have 4 characters left.  Then determine if they are
					//the correct characters for AI.
					if( idx < LenDataToEncode - 4 ) 
					{
						if ( (iCurrent2Chars <= 81 && iCurrent2Chars >= 80 ) || (iCurrent2Chars <= 34 && iCurrent2Chars >= 31) )
						{
							HRData = HRData + "(" + InputValue.Substring(idx + 1,4) + ")";
							idx = idx + 4;
						}
					}

					//Is this a 3 digit AI?
					if( idx < LenDataToEncode - 3 ) 
					{
						if( (iCurrent2Chars <= 49 && iCurrent2Chars >= 40) || (iCurrent2Chars <= 25 && iCurrent2Chars >= 23) )
						{
							HRData = HRData + "(" + InputValue.Substring(idx + 1,3) + ")";
							idx = idx + 3;
						}
					}
					
					//Is this a 2 digit AI?
					if( (iCurrent2Chars <= 30 && iCurrent2Chars >= 0) )
					{
						HRData = HRData + "(" + InputValue.Substring(idx + 1, 2) + ")";
						idx = idx + 2;
					}	
					else if( iCurrent2Chars <= 99 )
					{	
						if (iCurrent2Chars >=90)
						{
							HRData = HRData + "(" + InputValue.Substring(idx + 1, 2) + ")";
							idx = idx + 2;
						}
				...
				}	//End if iAsciiChar == 202
				else
				{
					if( CharAsciiValue < 32 || CharAsciiValue == 194 )
						//If the ascii character is less than 32, it is an unprintable character. Show it as a space.
						HRData = HRData + " ";
					else
					{
						//If it is a regular ascii value, just print it.
						if( CharAsciiValue > 31 && CharAsciiValue < 128 )
						{
							HRData = HRData + chrInputValue[idx];
						}
					}
				}//End if Check for FNC1 character
			} //End for loop
			return HRData;
		}//End Code128 HR
		...
		public string Codabar(string InputValue)
		{
			...
			/* Check to make sure data is numeric, $, +, -, /, or :, and remove all
			others. */
			for(idx = 0;idx < LenOfData;idx++)
			{
				CurrentChar = InputValue.Substring(idx,1);
				if( CurrentChar == "$" || CurrentChar == "+" || CurrentChar == "-" || CurrentChar == "/" 
					|| CurrentChar == ":" || IsNumber(CurrentChar) == true )
		...
		public string Code39mod43(string InputValue)
		{
			...
			for(idx=0;idx<LenOfData;idx++)
			{
				...
				if((iCurrentChar < 58) && (iCurrentChar > 47))
					/* 0-9 */
					OutData = OutData + cInputValue[idx];
				else if((iCurrentChar < 91) && (iCurrentChar > 64))
					/* A-Z */
					OutData = OutData + cInputValue[idx];
			...
			else if((CheckDigit < 36) && (CheckDigit > 9))
				/* A-Z */
				CheckDigit=CheckDigit + 55;
			else if(CheckDigit==38)
				/* Space */
				CheckDigit=61;
		...
		public string UPCA(string InputValue)
		{
			...
			if(EAN5AddOn.Length == 5)
			{
				...
				for(idx = 0;idx < LenOfData;idx++)
				{
					/* Get the actual value of each number */
					/* it is encoded with variable parity */
					iCharNumber = Convert.ToInt32(EAN5AddOn.Substring(idx,1));
					...
					/* add in the space & add-on guard pattern */
					/* and Now print add-on delineators between each add-on character */
					switch(idx)
					{
						case 0:
							iCharNumber = 43;

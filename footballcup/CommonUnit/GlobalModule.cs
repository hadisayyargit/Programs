...
namespace CommonUnit
{
    public class GlobalModule
    {
        ...
        public const string m_CurrentCup = "currentcup";

        public const string m_CompetitorList = "Competitorlist";

        public const string m_PredictionList = "predictionlist";

        public const string m_NotificationList = "messagelist";
        public const string m_NotificationCount = "messagecount";
        ...
        public static string Horof(string Number)
        {
            ...
            switch (N.Length)
            {
                ...
                case 2:
                case 3:
                    if (Convert.ToInt64(N) < 20) return (GlobalModule.AlphaNumeric1[Convert.ToInt64(N)]);
                    else
                        if (Convert.ToInt64(N) < 100)
                        if ((Convert.ToInt64(N) % 10) == 0) return (GlobalModule.AlphaNumeric2[(Convert.ToInt64(N) / 10)]);
                        else return (GlobalModule.AlphaNumeric2[Convert.ToInt64(N) / 10] + " و " + Horof((Convert.ToInt64(N) % 10).ToString()));
                    else if (Convert.ToInt64(N) < 1000)
                        if ((Convert.ToInt64(N) % 100) == 0) return (GlobalModule.AlphaNumeric3[Convert.ToInt64(N) / 100]);
                        else return (GlobalModule.AlphaNumeric3[Convert.ToInt64(N) / 100] + " و " + Horof((Convert.ToInt64(N) % 100).ToString()));
                    break;
                case 4:
                case 5:
                case 6:

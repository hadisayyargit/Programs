    ...
    public partial class frmPersonnel : Form
    {
        ...
        private void save()
        {
            ...
            catch (System.Exception ocException)
            {
                ...
                if (ocException.Message.Contains("IX_Prs_Tbl_Personel_Prs_Fld_PersonelID"))
                {
                    MessageBox.Show("اطلاعات شماره پرسنلی تکراری است", "خطا", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                if (ocException.Message.Contains("IX_Prs_Tbl_Personel_National_code"))
                {
                    MessageBox.Show("اطلاعات کد ملی تکراری است", "خطا", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                } 
        ...
        private bool IsValidData()
        {
            ...
            try
            {
                nYear = int.Parse(txtBirthYear.Text);
            }
            catch
            {
            }

            if (nYear < 1300 || nYear > 1390)
            {
                ProgLib.Components.MessageBox.Show("تاریخ تولد نامعتبر است", "اخطار", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                txtBirthYear.Focus();
                return false;
            }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (ProgLib.Components.MessageBox.Show(CMessageHandler.Handle(Program.m_System, 106), "هشدار", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                {
                    if (strRes.Contains("IX_Prs_Tbl_Personel_UniqueKey1"))
                    {

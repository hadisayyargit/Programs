...
{
    public partial class frmForfeitKind : Form
    {
        public frmForfeitKind()
        {
            InitializeComponent();
        }
        ...
        private bool Prevalidation()
        {
            ...
            if (byte.TryParse(txtCode.Text,out  n1)==false)
            {
                Cursor = Cursors.Default;
                errorProvider1.SetError(txtCode, "کد را بشکل عدد کوچکتر از 256 وارد نماييد");
                bRes = false;
                return bRes;
            }
        ...
        private void RefreshData()
        {
            ...
            m_dvMaster = ForfeitKind.GetForfeitKind();
        ...
        {
            string strRes = "";
            ForfeitKind objForfeitKind = new ForfeitKind();

            objForfeitKind.ForfeitKindId = Convert.ToByte(txtCode.Text);
            objForfeitKind.Pname = txtName.Text;

            strRes = objForfeitKind.dbInsert();
            if (strRes == "")
                MessageBox.Show("اطلاعات با موفقيت اضافه شد", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else
            {
                if (strRes.Contains("IX_Rws_Tbl_ForfeitKind_UniqueKey1"))
                {
                    MessageBox.Show("اطلاعات تکراری است", "خطا", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
            ...
            nRow = getGridRowIndex(objForfeitKind.ForfeitKindId.ToString());
            if (nRow >= 0)
            {
        ...
        void UpdateData()
        {
            ...
            strRes = objForfeitKind.dbUpdate();
            if (strRes == "")
        ...
        void deleteData(byte nForfeitKind)
        {
            string strRes = "";
            ForfeitKind objForfeitKind = new ForfeitKind();

            objForfeitKind.ForfeitKindId = nForfeitKind;

            try
            {
                strRes = objForfeitKind.dbDelete();
            }
        ...
        private void frmForfeitKind_Load(object sender, EventArgs e)
        {
            Program.m_PrgMod = Program.PrgMod.Normal;
            RefreshData();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            //if (grdMaster.Rows.Count > 0)
            {
                ...
                {
                    deleteData(Convert.ToByte(grdMaster["colCode", grdMaster.CurrentRow.Index].Value.ToString()));
                }
            }
